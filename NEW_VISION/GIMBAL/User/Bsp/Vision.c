...
void VisionSendInit(union RUI_U_VISION_SEND*  Send_t,TYPEDEF_VISION *VISION_DATA,User_Data_T*User_Data_aaa,DBUS_Typedef *DBUS_sss,IMU_Data_t *IMU_Data,MOTOR_Typdef*Motor_t)
{
...
/// @param type 类型 0:虚拟串口 1:USART1
/// @return 
int ControltoVision(union RUI_U_VISION_SEND*  Send_t , uint8_t *buff, uint8_t type,User_Data_T *users,DBUS_Typedef*DBUS,IMU_Data_t *IMU_Data,TYPEDEF_VISION *VISION_V_DATA,MOTOR_Typdef *ALL_MOTOR)
{
    uint8_t status;
   VisionSendInit(Send_t,VISION_V_DATA,users,DBUS,IMU_Data,ALL_MOTOR);
    buff[0] = 0xcd;
	//pitch
