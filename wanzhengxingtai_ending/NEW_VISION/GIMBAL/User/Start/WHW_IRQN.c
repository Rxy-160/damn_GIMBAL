...
//                              &CAPDATE.GET, &ALL_MOTOR);
        ...
        RobotTask(2, &WHW_V_DBUS, &RUI_V_CONTAL, &User_data,
                 &CAPDATE, &VISION_V_DATA, &RUI_ROOT_STATUS, &ALL_MOTOR,&IMU_Data,&TDDDD,&VT13_DBUS);
...
//		///////普通视觉发送
	 ControltoVision(&VISION_V_DATA.SEND ,sd_v_buff, 1,&User_data,&WHW_V_DBUS,&IMU_Data ,&VISION_V_DATA,&ALL_MOTOR );
///加预测视觉发送
...
// uint16_t data_length = sizeof(VT13_RX_DATA) - __HAL_DMA_GET_COUNTER(&hdma_usart6_rx);
//					if(data_length>0)
//					{
//					}
            ...
            // 下面进行空闲中断相关处理
					            ...
					            HAL_UART_Receive_DMA(&huart6, (uint8_t *)VT13_RX_DATA,sizeof(VT13_RX_DATA));  //重启开始DMA传输
