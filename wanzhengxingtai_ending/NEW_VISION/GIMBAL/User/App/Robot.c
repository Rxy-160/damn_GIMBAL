...
//            {
                ...
                if(DBUS->Remote .S1_u8 ==1||DBUS->Remote .S1_u8 ==2||VT13_DBUS->KeyBoard .Ctrl ==0)
								{
									CONTAL->MOD[0] = 1;//云台模式切换///////手瞄模式
                CONTAL->HEAD.Pitch -= (float) (DBUS->Remote.CH3_int16) * 0.04f +
                                      VT13_DBUS->Mouse.Y_Flt * 2.0f;
                ...
                CONTAL->HEAD.Yaw  -= (float) (DBUS->Remote.CH2_int16) * 0.006f +
                                    RUI_F_MATH_Limit_float(1, -1, VT13_DBUS->Mouse.X_Flt * 0.3f) +
                                    (float) (VT13_DBUS->KeyBoard.E - VT13_DBUS->KeyBoard.Q)*2.8;
								}
								else if(DBUS->Remote .S1_u8 ==3||VT13_DBUS->KeyBoard .Ctrl ==1)
								{
