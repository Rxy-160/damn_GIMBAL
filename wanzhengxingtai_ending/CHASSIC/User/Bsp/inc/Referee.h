...
enum Read_Cmd_ID_Typdef
{
    ...
    Radar_Informat  = 0x020E ,  //雷达自主决策信息同步，固定1Hz 频率发送
		Interactive_Data= 0x0301 ,	//机器人交互数据，发送方触发发送，频率上限为 30Hz 
		Custom_Controller=0x0302 ,  //自定义控制器与机器人交互数据，发送方触发发送，频率上限为 30Hz 
...
{
    uint8_t game_type : 4;      //比赛类型    bit0-3
    uint8_t game_progress : 4;  //当前比赛阶段bit4-7
...
//}game_robot_HP_t;///////////////老一版的接收裁判系统血量数据
typedef struct _packed
{ 
uint16_t ally_1_robot_HP; //己方 1 号英雄机器人血量，若该机器人未上场或者被罚下，则血量为 0
...
typedef struct __packed
{
    uint32_t supply_Blood : 1;              //己方与兑换区不重叠的补给区占领状态，1为已占领
    uint32_t exchange_Blood : 1;            //己方与兑换区重叠的补给区占领状态，1为已占领
    uint32_t supply_Blood_RMUL : 1;         //己方补给区的占领状态，1 为已占领（仅 RMUL 适用）
    
		uint32_t buff_small : 2;                //己方小能量机关的激活状态，1 为已激活，2为正在激活
		uint32_t buff_big : 2;                  //己方大能量机关的激活状态，1 为已激活，2为正在激活
	
    uint32_t Center_Heights : 2;            //己方中央高地的占领状态，1 为被己方占领，2 为被对方占领
    uint32_t Trapezoidal_heights : 2;       //己方梯形高地的占领状态，1 为被己方占领，2 为被对方占领
    uint32_t Dart_hit_time : 9;             //飞镖最后一次击中己方前哨站或基地的时间（0-420，开局默认为0
	uint32_t Dart_hit_aim : 3;              //飞镖最后一次击中己方前哨站或基地的具体目标，开局默认为 0，1 为击中前哨站，2 为击中基地固定目标，3 为击中基地随机固定目标，4为击中基地随机移动目标，5为击中基地末端移动目标
	
    uint32_t Center_Buff_RMUL : 2;          //中心增益点的占领情况，0 为未被占领，1 为被己方占领，2 为被对方占领，3 为被双方占领。（仅 RMUL 适用）
		uint32_t fort_buff : 2;                 // 己方堡垒增益点占领状态，0=未被占领，1=被己方占领，2=被对方占领，3=被双方占领
		uint32_t outpost_buff : 2;              // bit 27-28: 己方前哨站增益点占领状态，0=未被占领，1=被己方占领，2=被对方占领
		uint32_t base_buff : 1;                 // bit 29: 己方基地增益点占领状态，1=已占领
	
    uint32_t Receive : 2;                   //保留位
}event_data_t;
/////已更新
...
typedef struct __packed
{
    ...
    uint16_t Dart_aimed : 3;         //最近一次己方飞镖击中的目标，开局默认为 0，1 为击中前哨站，2 为击中基地固定目标，3为击中基地随机固定目标，4为击中基地随机移动目标，5 为击中基地末端移动目标
    uint16_t Dart_success : 3;       //对方最近被击中的目标累计被击中计数，开局默认为 0，至多为 4
    uint16_t Dart_aiming : 2;        //飞镖此时选定的击打目标，开局默认或未选定/选定前哨站时为 0，选中基地固定目标为 1，选中基地随机目标为 2，选中基地随机移动目标为 3，选中基地末端移动目标为 4 
    uint16_t Reserved_bits : 8;      //保留
...
typedef struct __packed
{
    ...
    uint16_t cooling_buff;       //机器人枪口冷却倍率（直接值，值为 5 表示 5 倍冷却）
    ...
    uint8_t remaining_energy_125: 1;//在剩余能量≥125%时为 1，其余情况为 0
    uint8_t remaining_energy_100: 1;//在剩余能量≥100%时为 1，其余情况为 0 
...
typedef struct __packed
{
    ...
    uint32_t bit6 : 1;      //己方地形跨越增益点（飞坡）（靠近己方一侧飞坡后）
    ...
    uint32_t bit21 : 1;     //己方装配增益点（原：己方大资源岛增益点）
		
    uint32_t bit22 : 1;     //对方装配增益点（原：对方大资源岛增益点）
    uint32_t bit23 : 1;     //中心增益点（仅 RMUL 适用）
    uint32_t bit24 : 1; 		//对方堡垒增益点（新增）
		uint32_t bit25 : 1;			//对方前哨站增益点（新增）
		...
		uint8_t bit32 : 1;			//对方地形跨越增益点（隧道）（靠近对方公路一侧下方）
		uint8_t bit33 : 1;			//对方地形跨越增益点（隧道）（靠近对方公路一侧中间）
		uint8_t bit34 : 1;			//对方地形跨越增益点（隧道）（靠近对方公路一侧上方）
		uint8_t bit35 : 1;			//对方地形跨越增益点（隧道）（靠近对方梯形高地较低处）
		uint8_t bit36 : 1;			//对方地形跨越增益点（隧道）（靠近对方梯形高地较中间）
		uint8_t bit37 : 1;			//对方地形跨越增益点（隧道）（靠近对方梯形高地较高处）
		uint8_t bit38_63 : 2;	//保留位
}rfid_status_t;
...
typedef struct __packed
{
//    uint8_t mark_hero_progress : 1;     //对方英雄机器人易伤情况,100时发送1，被标记进度<100时发送0
//    uint8_t mark_engineer_progress : 1;     //对方工程机器人易伤情况,100时发送1，被标记进度<100时发送0
//    uint8_t mark_standard_3_progress : 1;   //对方 3 号步兵机器人易伤情况,100时发送1，被标记进度<100时发送0
//    uint8_t mark_standard_4_progress : 1;   //对方 4 号步兵机器人易伤情况,100时发送1，被标记进度<100时发送0
////    uint8_t mark_standard_5_progress;   //对方 5 号步兵机器人易伤情况,100时发送1，被标记进度<100时发送0
//    uint8_t mark_sentry_progress : 1;       //对方哨兵机器人易伤情况,100时发送1，被标记进度<100时发送0
		uint16_t opp_hero_marked : 1;           // bit 0: 对方1号英雄机器人易伤情况
    uint16_t opp_engineer_marked : 1;       // bit 1: 对方2号工程机器人易伤情况
    uint16_t opp_infantry_3_marked : 1;     // bit 2: 对方3号步兵机器人易伤情况
    uint16_t opp_infantry_4_marked : 1;     // bit 3: 对方4号步兵机器人易伤情况
    uint16_t opp_aerial_marked : 1;         // bit 4: 对方空中机器人特殊标识情况（新增）
		uint16_t opp_sentry_marked : 1;         // bit 5: 对方哨兵机器人易伤情况
		uint16_t self_hero_marked : 1;          // bit 6: 己方1号英雄机器人特殊标识情况（新增）
    uint16_t self_engineer_marked : 1;      // bit 7: 己方2号工程机器人特殊标识情况（新增）
    uint16_t self_infantry_3_marked : 1;    // bit 8: 己方3号步兵机器人特殊标识情况（新增）
    uint16_t self_infantry_4_marked : 1;    // bit 9: 己方4号步兵机器人特殊标识情况（新增）
    uint16_t self_aerial_marked : 1;        // bit 10: 己方空中机器人特殊标识情况（新增）
    uint16_t self_sentry_marked : 1;        // bit 11: 己方哨兵机器人特殊标识情况（新增）
...
typedef struct __packed
{
uint32_t allowed_bullet_redeemed : 11;      // bit 0-10: 除远程兑换外，哨兵机器人成功兑换的允许发弹量，开局为0    uint32_t remote_successfully_redeemed : 4;  //哨兵成功远程兑换发弹量的次数，开局为 0，在哨兵成功远程兑换发弹量后，该值将变为哨兵成功远程兑换发弹量的次数。 
uint32_t remote_bullet_redeem_count : 4;    // bit 11-14: 哨兵机器人成功远程兑换允许发弹量的次数，开局为0    uint32_t FreeRevive : 1;                    //哨兵兵机器人当前是否可以兑换立即复活，可以兑换立即复活时值为1，否则为0
uint32_t remote_hp_redeem_count : 4;        // bit 15-18: 哨兵机器人成功远程兑换血量的次数，开局为0    uint32_t PayReviveNum : 10;                 //哨兵机器人当前若兑换立即复活需要花费的金币数
uint32_t free_revive_available : 1;         // bit 19: 哨兵机器人当前是否可以确认免费复活，1=可确认，0=不可
uint32_t pay_revive_available : 1;          // bit 20: 哨兵机器人当前是否可以兑换立即复活，1=可兑换，0=不可
uint32_t pay_revive_cost : 10;              // bit 21-30: 哨兵机器人当前若兑换立即复活需要花费的金币数	
uint32_t reserved_0 : 1;                    // bit 31: 保留位	
	
uint16_t out_of_combat : 1;                 //  哨兵当前是否处于脱战状态，1=脱战，0=战斗中
uint16_t bullet_redeem_remaining :11;				//队伍17mm允许发弹量的剩余可兑换数
uint16_t sentry_posture :2		;		  				//哨兵当前姿态，1=进攻，2=防御，3=移动
uint16_t power_rune_activatable :1;        //己方能量机关是否能够进入正在激活状态，1=当前可激活
...
typedef struct __packed
{
	...
	uint8_t self_vulnerable_active  : 1;//方是否正在被对方触发双倍易伤，1=正在触发，0=未触发
	uint8_t vulnerable_remaining_time :2;//当前双倍易伤剩余时间档位，0=无，1=0-5s，2=5-10s，3=10-15s
