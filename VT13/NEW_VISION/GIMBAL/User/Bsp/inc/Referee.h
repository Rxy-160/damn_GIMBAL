...
typedef struct __packed
{
    ...
    uint32_t Dart_hit_aim : 3;              //飞镖最后一次击中己方前哨站或基地的具体目标，开局默认为 0，1 为击中前哨站，2 为击中基地固定目标，3 为击中基地随机固定目标，4为击中基地随机移动目标
    uint32_t Center_Buff_RMUL : 2;          //中心增益点的占领情况，0 为未被占领，1 为被己方占领，2 为被对方占领，3 为被双方占领。（仅 RMUL 适用）
    uint32_t Receive : 9;                   //保留位
}event_data_t;
...
typedef struct __packed
{
    ...
    uint16_t Dart_aimed : 3;         //最近一次己方飞镖击中的目标，开局默认为 0，1 为击中前哨站，2 为击中基地固定目标，3为击中基地随机固定目标，4为击中基地随机移动目标
    uint16_t Dart_success : 3;       //对方最近被击中的目标累计被击中计数，开局默认为 0，至多为 4
    uint16_t Dart_aiming : 2;        //飞镖此时选定的击打目标，开局默认或未选定/选定前哨站时为 0，选中基地固定目标为 1，选中基地随机目标为 2
...
typedef struct __packed
{
    uint16_t chassis_voltage_reserved;               //电源管理模块的 chassis 口输出电压（单位：mV）
    uint16_t chassis_current_reserved;               //电源管理模块的 chassis 口输出电流（单位：mA）
...
typedef struct __packed
{
    ...
    uint8_t remaining_energy_100: 3;//保留
}buff_t;
...
typedef struct __packed
{
    ...
    uint32_t bit19 : 1;     //己方与兑换区不重叠的补给区/RMUL补给区
    uint32_t bit20 : 1;     //己方与兑换区重叠的补给区
    uint32_t bit21 : 1;     //己方大资源岛增益点
    uint32_t bit22 : 1;     //对方大资源岛增益点
    uint32_t bit23 : 1;     //中心增益点（仅 RMUL 适用）
    uint32_t bit24_31 : 8; //保留
...
typedef struct __packed
{
    ...
    uint32_t FreeRevive : 1;                    //哨兵兵机器人当前是否可以兑换立即复活，可以兑换立即复活时值为1，否则为0
    uint32_t PayRevive : 1;                     //：哨兵机器人当前是否可以兑换立即复活，可以兑换立即复活时值为1，否则为0
    uint32_t PayReviveNum : 10;                 //哨兵机器人当前若兑换立即复活需要花费的金币数
