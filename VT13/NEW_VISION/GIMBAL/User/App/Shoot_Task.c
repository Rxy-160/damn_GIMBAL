...
//             |Trapezoid_Intergral|ChangingIntegrationRate//梯形积分,变速积分
//             |Derivative_On_Measurement|DerivativeFilter);//微分先行,微分滤波器
		    PID_Init(&MOTOR->DJI_3508_Shoot_M.PID_P , 15000.0f, 2000.0f,
             PID_P_M, 0, 0,
             0, 0, 0,
             Integral_Limit|ErrorHandle//积分限幅,输出滤波,堵转监测
             //梯形积分,变速积分
...
//                          MOTOR->DJI_3508_Shoot_M.DATA.Aim);
    ...
    DJI_Current_Ctrl(&hcan2,
                     ...
                     0,
                     0);
			DJI_Current_Ctrl(&hcan1,0x200,0,0,(int16_t)tmp_S[2],0);
...
//        MOTOR->DJI_3508_Shoot_R.PID_S.IntegralLimit = 0;
        MOTOR->DJI_3508_Shoot_R.DATA.Aim = 0;//(float)MOTOR->DJI_3508_Shoot_R.DATA.Speed_now;

//        MOTOR->DJI_3508_Shoot_M.PID_P.IntegralLimit = 0;
//        MOTOR->DJI_3508_Shoot_M.PID_S.IntegralLimit = 0;
        MOTOR->DJI_3508_Shoot_M.DATA.Aim = 0;//(float)MOTOR->DJI_3508_Shoot_M .DATA.Angle_now ;
...
void ATTACK_F_JAM_Aim(MOTOR_Typdef *MOTOR, VT13_Typedef *VT13_DBUS, uint8_t autofire)
{
    ...
    // 单发模式处理 (通过状态变化检测)
    if (VT13_DBUS->Remote .fn_1 == 1)//这一次是单发
    {
        // 检测遥控器模式切换到单发     2
...
//		}
    ...
    ATTACK_V_PARAM.SPEED = 6700.0f ;//+ TEMP;7200----25.8   6700--23.1/23.2/23.3
//    ATTACK_V_PARAM.SPEED = MATH_D_LIMIT(6370.0f, 6180.0f, ATTACK_V_PARAM.SPEED);
    ...
    if (/*( fire_mouse_status == 1 && DBUS->Remote .S1_u8 == 3)||*/ VT13_DBUS->Remote.mode_sw  ==1||VT13_DBUS->Remote.mode_sw  == 2)  // 3 is the fire button
        ...
        MOTOR->DJI_3508_Shoot_L .DATA .Aim =  (ATTACK_V_PARAM.SPEED);
			...
			//////////摩擦轮缓启动
//					MOTOR->DJI_3508_Shoot_L .DATA .Aim +=acceleration_fire;
//					MOTOR->DJI_3508_Shoot_R .DATA .Aim -=acceleration_fire;
//					if (MOTOR->DJI_3508_Shoot_L .DATA .Aim >ATTACK_V_PARAM.SPEED)
//					{
//						MOTOR->DJI_3508_Shoot_L .DATA .Aim =ATTACK_V_PARAM.SPEED;
//					}
//					if (MOTOR->DJI_3508_Shoot_R .DATA .Aim < -(ATTACK_V_PARAM.SPEED+30) )
//					{
//						MOTOR->DJI_3508_Shoot_R .DATA .Aim =-(ATTACK_V_PARAM.SPEED+30);
//					}
				/////////	
...
//   // }
 //计算拨弹电机的目标值
   		ATTACK_F_JAM_Aim(&ALL_MOTOR ,VT13_DBUS ,0);//最后一位没用
 // shooting case when opposite to you l-r
		//计算两个摩擦轮目标值
    ATTACK_F_FIRE_Aim(&ALL_MOTOR ,VT13_DBUS );
 ...
 */
float ATTACK_F_FireRate_Control(MOTOR_Typdef *motor, float hz, uint8_t type)
{
    switch (type)
    {
    case 1: {// 线性拟合
...
//        compensation_hz = ATTACK_Calc_Hz_From_FactHz(hz, a, P);
        compensation_hz = RUI_F_MATH_Limit_float(23.0f, 12.0f, compensation_hz); // 限制最大值
        motor->DJI_3508_Shoot_M .PID_P .MaxOut  = compensation_hz * 60.0f * 4.5f;
        break;
    }
    case 4: {
        motor->DJI_3508_Shoot_M .PID_P .MaxOut = hz * 60.0f * 4.5f;
    }
    default:
        break;
    }
    return motor->DJI_3508_Shoot_M .PID_P .MaxOut ;
}
...
uint8_t ATTACK_F_HeatControl(MOTOR_Typdef *MOTOR, uint8_t type,User_Data_T *User_Data) 
{
    ...
    ATTACK_F_FireRate_Control(&ALL_MOTOR, 18.0f, 3);
...
uint8_t ATTACK_F_JAM_Check(MOTOR_Typdef *MOTOR)
{
    float DIFF = fabsf(MOTOR->DJI_3508_Shoot_M .DATA .Aim  - (float)MOTOR->DJI_3508_Shoot_M .DATA .Angle_Infinite );
    float EDGE = (float)(ATTACK_V_PARAM.SINGLE_ANGLE / 5.0f);

    if (((DIFF >= EDGE) && ((fabsf((float)MOTOR->DJI_3508_Shoot_M .DATA .Speed_now )) <= ATTACK_D_SPEED)))
    {
