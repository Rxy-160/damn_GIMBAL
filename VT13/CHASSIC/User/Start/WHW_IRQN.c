...
void StartMoveTask(void const * argument)
{
    ...
    //Pitch轴限幅
	 //
	/////////注意p轴初始化
    RUI_V_CONTAL.HEAD.Pitch_MAX = 300;
    RUI_V_CONTAL.HEAD.Pitch_MIN = -550;

    for (;;)
    {
        /*底盘*/
        RobotTask(1, &WHW_V_DBUS, &RUI_V_CONTAL, &User_data,
                  &cap, &VISION_V_DATA, &RUI_ROOT_STATUS, &ALL_MOTOR,&IMU_Data);
...
/////////双板
    ...
    if (RUI_V_CONTAL.CG.RELATIVE_ANGLE > 4096)
    {
        RUI_V_CONTAL.CG.RELATIVE_ANGLE -= 8192;
    }
    else if (RUI_V_CONTAL.CG.RELATIVE_ANGLE < -4096)
    {
        RUI_V_CONTAL.CG.RELATIVE_ANGLE += 8192;
    }
...
void StartIMUTask(void const * argument)
{
    ...
    for(;;)
    {
        ...
        ControltoVision(&VISION_V_DATA.SEND ,sd_v_buff, 1,&User_data,&WHW_V_DBUS,&IMU_Data );
...
//            HAL_UART_Receive_DMA(&huart6,(uint8_t *)ALL_RX.Data,sizeof(ALL_RX.Data));  //重启开始DMA传输
    ...
    {
		//数据处理
//        VISION_F_Cal(VISION_V_DATA.OriginData,1,&VISION_V_DATA);
